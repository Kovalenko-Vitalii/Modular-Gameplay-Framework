...
public class UIWindowManager : MonoBehaviour
{
    ...
    private void HandleBack(UIWindowDefinition fallback)
    {
        if (Top != null)
        {
            if (Top.closableWithEsc) Close(Top);
        }
    ...
    {
        if (window == null || stack.Contains(window)) return;
        stack.Add(window);
        GameLog.Log(TAG, "Opened window " + window.Id);
    ...
    public void Close(UIWindowDefinition window)
    {
        ...
        GameLog.Log(TAG, "Closed window " + window.Id);
    ...
    public void CloseAll()
    {
        ...
        {
            GameLog.Log(TAG, "Closed window " + w.Id);
    ...
    {
        if (string.IsNullOrEmpty(pauseReason)) return;
